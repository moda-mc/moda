    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length > 0) {
			...
			if (target == null) {
				sender.spigot().sendMessage(new ComponentBuilder("")
						.append(Moda.getPrefix())
						.append(Colors.toComponent(Moda.messages.getString("ping.errors.target-invalid")))
			...
			try {
				...
				player.spigot().sendMessage(new ComponentBuilder("")
						.append(Moda.getPrefix())
						.append(Colors.toComponent(Moda.messages.getString("ping.responses.others.ping")
								.replace("{ping}", ping + "")
        ...
        if (label.equalsIgnoreCase("hi")) {
            player.spigot().sendMessage(
                    ...
                    .append(Moda.getPrefix())
                    .append(Colors.toComponent(Moda.messages.getString("ping.responses.self.hi").replace("{ping}", ping + "")))
                    .create());
            return true;
		}
		player.spigot().sendMessage(new ComponentBuilder("")
				.append(Moda.getPrefix())
				.append(Colors.toComponent(Moda.messages.getString("ping.responses.self.ping").replace("{ping}", ping + "")))
				.create());
        return true;
    }
}
