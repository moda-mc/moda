...
import com.mineglade.moda.utils.IdiotException;
...
public class ColorPickerMenu extends IconMenu {
	...
	public ColorPickerMenu(Player player, OfflinePlayer target, IColorType type) throws IdiotException {
		super(Moda.instance, StringUtils.capitalize(type.toString().toLowerCase()) + " Colors", 3 * 9, player);
		this.target = target;
		this.player = player;
		this.type = type;
				...
				if (event.getName().contains("Reset")) {
					...
					if (!player.hasPermission(color.getPermission(IColorType.NAME))) {
						player.spigot().sendMessage(new ComponentBuilder("")
								...
								.append(Colors.toComponent(Moda.messages.getString("color.errors.no-permission")
		...
		if (type == IColorType.CHAT) {
			if (player.equals(target)) {
				if (event.getName().contains("Reset")) {
					...
					player.sendMessage(
							Moda.getPrefix() + Colors.parseColors(Moda.messages.getString("color.chat-color.set.self")
									.replace("{color}", "&" + color.getColorCode() + color.getName())));
					return true;
				}
			} else {
				if (event.getName().contains("Reset")) {
					data.resetChatColor();
					try {
						player.sendMessage(Moda.getPrefix() + Colors.parseColors(Moda.messages
					...
					try {
						player.sendMessage(
								Moda.getPrefix() + Colors.parseColors(Moda.messages.getString("color.chat-color.set.others")
										.replace("{color}", "&" + color.getColorCode() + color.getName())
										.replace("{target}", data.getNickName())));
					} catch (PlayerNotLoggedException e) {
						throw new IdiotException();
