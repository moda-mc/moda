...
import com.mineglade.moda.utils.CommandUtil;
...
public class CoreCommand implements CommandExecutor {
	...
	public boolean onCommand(final CommandSender sender, final Command command, final String label,
		...
		if (args.length < 1) {
			...
			sender.sendMessage(Moda.getPrefix() + ChatColor.DARK_GRAY + "==== " + ChatColor.GREEN
			...
			if (!sender.hasPermission(permission)) {
				sender.spigot()
				.sendMessage(new ComponentBuilder("").append(Moda.getPrefix())
			...
			Moda.instance.reloadConfig();
			sender.sendMessage(Moda.getPrefix() + ChatColor.GREEN
					+ "all iCore configs have been reloaded.");
			Moda.instance.initDataBaseConnection();
			sender.sendMessage(Moda.getPrefix() + ChatColor.GREEN
					+ "all database connections have been reloaded.");
			if (Moda.discord != null) {
				Bukkit.getScheduler().runTaskAsynchronously(Moda.instance, Moda.discord::restart);
				sender.sendMessage(Moda.getPrefix() + ChatColor.GREEN
